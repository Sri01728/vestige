...
pub async fn execute(
    ...
    if let Some(ws) = super::workspace::get_active_workspace(storage) {
    ...
    auto_categorize_tags(&content, &mut tags);
    extract_entities(&content, &mut tags);
    ...
    #[cfg(all(feature = "embeddings", feature = "vector-search"))]
    {
        ...
        let conflict = result.superseded_id.as_ref().and_then(|sid| {
            storage.get_node(sid).ok().flatten().map(|old| {
                serde_json::json!({
                    "supersededId": sid,
                    "oldContent": old.content,
                    "oldRetention": old.retention_strength,
                    "whySuperseded": result.reason,
                })
            })
        });
...
async fn execute_batch(
    ...
    for (i, item) in items.into_iter().enumerate() {
        ...
        auto_categorize_tags(&item.content, &mut tags);
        extract_entities(&item.content, &mut tags);
...
fn auto_categorize_tags(content: &str, tags: &mut Vec<String>) {
    // Collect existing prefixes upfront to avoid borrow conflicts
...
fn extract_entities(content: &str, tags: &mut Vec<String>) {
    let manual_count = tags.iter().filter(|t| !t.starts_with("intent:")).count();
    if manual_count >= 3 { return; }
    ...
    for word in content.split_whitespace() {
        ...
        // File paths: contains / and an extension
        if clean.contains('/') && clean.contains('.') && clean.len() > 5 {
            let tag = format!("entity:{}", clean);
            if tag.len() <= 80 && !tags.contains(&tag) {
                tags.push(tag);
            }
            continue;
        }

        // CamelCase identifiers (e.g. UserService, AuthController)
        let chars: Vec<char> = clean.chars().collect();
        let upper_count = chars.iter().filter(|c| c.is_uppercase()).count();
        let lower_count = chars.iter().filter(|c| c.is_lowercase()).count();
        if upper_count >= 2 && lower_count >= 2 && chars[0].is_uppercase() && clean.len() >= 6 {
            let tag = format!("entity:{}", clean);
            if tag.len() <= 80 && !tags.contains(&tag) {
                tags.push(tag);
            }
        ...
        // snake_case identifiers with at least one underscore (e.g. user_service, auth_token)
        if clean.contains('_') && clean.chars().all(|c| c.is_alphanumeric() || c == '_') && clean.len() >= 6 {
            let tag = format!("entity:{}", clean);
            if tag.len() <= 80 && !tags.contains(&tag) {
                tags.push(tag);
            }
    ...
    // Cap entity tags to avoid bloat
    let entity_count = tags.iter().filter(|t| t.starts_with("entity:")).count();
    if entity_count > 10 {
        tags.retain(|t| !t.starts_with("entity:"));
    }
}
