...
pub async fn execute(
    ...
    let active_workspace = super::workspace::get_active_workspace(storage);
    if let Some(ref ws) = active_workspace {
        let ws_line = format!("**Workspace:** {}", ws);
        context_parts.push(ws_line.clone());
        char_count += ws_line.len() + 1;
    }

    // ====================================================================
    // 0. Core memories — always loaded first, regardless of query
    // ====================================================================
    if let Ok(core_memories) = storage.get_core_memories() {
        if !core_memories.is_empty() {
            let mut core_lines: Vec<String> = Vec::new();
            for m in &core_memories {
                seen_ids.insert(m.id.clone());
                let line = format!("- \u{1f4cc} {}", first_sentence(&m.content));
    ...
    // ====================================================================
    // 0b. Disclosure triggers — memories matching current context
    // ====================================================================
    if let Some(ref ctx) = args.context {
        let mut terms: Vec<&str> = Vec::new();
        if let Some(ref topics) = ctx.topics {
            for t in topics {
                terms.push(t.as_str());
            }
        }
        if let Some(ref codebase) = ctx.codebase {
            terms.push(codebase.as_str());
        }
        if !terms.is_empty() {
            if let Ok(triggered) = storage.get_triggered_memories(&terms) {
                ...
                for m in &triggered {
                    ...
                    seen_ids.insert(m.id.clone());
                    let line = format!("- \u{1f514} {}", first_sentence(&m.content));
                    let line_len = line.len() + 1;
                    if char_count + line_len <= budget_chars {
    ...
    // ====================================================================
    // 3b. Health alerts — important memories about to decay
    // ====================================================================
    if include_status {
        if let Ok(at_risk) = storage.get_at_risk_memories(5) {
            if !at_risk.is_empty() {
                let alert_lines: Vec<String> = at_risk
                    .iter()
                    .map(|m| {
                        format!(
                            "- \u{26a0} {:.0}% — {}",
                            m.retention_strength * 100.0,
                            first_sentence(&m.content)
                        )
                    })
                    .collect();
                let alert_section = format!(
                    "**At Risk ({}):**\n{}",
                    at_risk.len(),
                    alert_lines.join("\n")
                );
                let alert_len = alert_section.len() + 1;
                if char_count + alert_len <= budget_chars {
                    context_parts.push(alert_section);
                    char_count += alert_len;
                }
            }
        }
    }
