...
enum Commands {
    ...
    /// Quick add a memory (alias for ingest)
    Add {
        /// Content to remember
        content: String,
        /// Tags (comma-separated)
    ...
    Workspace {
        /// Action: set, get, list, new, clear
        action: String,
        /// Workspace name (for set/new)
        name: Option<String>,
    },
...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Add { content, tags } => run_ingest(content, tags, "fact".to_string(), None),
        Commands::Workspace { action, name } => run_workspace(action, name),
        Commands::Serve {
            port,
            dashboard,
            dashboard_port,
...
fn run_search(query: String, limit: i32) -> anyhow::Result<()> {
    ...
    println!("{}", "=== Vestige Search ===".cyan().bold());
    println!(
        "{}: \"{}\"  ({} results)",
        "Query".white().bold(),
        query,
    ...
    for (i, r) in results.iter().enumerate() {
        let retention_pct = format!("{:.0}%", r.node.retention_strength * 100.0);
        let retention_colored = if r.node.retention_strength >= 0.7 {
            retention_pct.green()
        } else if r.node.retention_strength >= 0.4 {
            retention_pct.yellow()
        } else {
            retention_pct.red()
        };
        ...
        println!(
            ...
            truncate(&r.node.content, 80),
            retention_colored,
            r.node.id.dimmed()
        );

        if !r.node.tags.is_empty() {
            println!(
                "   {}",
                r.node
                    .tags
...
fn run_ingest(
    ...
    // Auto-categorize tags (same logic as MCP smart_ingest tool)
    auto_categorize_tags(&content, &mut tag_list);

    let storage = Storage::new(None)?;

    // Auto-tag with active workspace
    if let Ok(Some(ws)) = storage.get_config("active_workspace") {
        if !ws.is_empty() {
            let project_tag = format!("project:{}", ws);
            if !tag_list.contains(&project_tag) {
                tag_list.push(project_tag);
            }
        }
    }
...
fn run_workspace(action: String, name: Option<String>) -> anyhow::Result<()> {
    let storage = Storage::new(None)?;

    match action.as_str() {
        "get" => {
            let ws = storage.get_config("active_workspace")?;
            match ws.filter(|s| !s.is_empty()) {
                Some(w) => println!("{} {}", "Active workspace:".white().bold(), w.green().bold()),
                None => println!("{}", "No active workspace (global mode)".dimmed()),
            }
        }
        "set" => {
            let name = name.ok_or_else(|| anyhow::anyhow!("Usage: vestige workspace set <name>"))?;
            storage.set_config("active_workspace", &name.to_lowercase())?;
            println!("{} {}", "Workspace set to:".white().bold(), name.to_lowercase().green().bold());
            println!("{}", "All new memories will be auto-tagged with project:".dimmed());
        }
        "new" => {
            let name = name.ok_or_else(|| anyhow::anyhow!("Usage: vestige workspace new <name>"))?;
            storage.set_config("active_workspace", &name.to_lowercase())?;
            println!("{} {}", "Created workspace:".white().bold(), name.to_lowercase().green().bold());
            println!("{}", "Workspace activated. All new memories auto-tagged.".dimmed());
        }
        "clear" => {
            storage.set_config("active_workspace", "")?;
            println!("{}", "Workspace cleared. Back to global mode.".yellow());
        }
        "list" => {
            let tags = storage.list_tags()?;
            let active = storage.get_config("active_workspace")?.unwrap_or_default();
            let mut found = false;
            ...
            for (tag, count) in &tags {
                if let Some(name) = tag.strip_prefix("project:").or_else(|| tag.strip_prefix("codebase:")) {
                    ...
                    println!("  {} — {} memories{}", name.white().bold(), count, marker);
                    found = true;
                }
            }
            if !found {
                println!("  {}", "No workspaces yet. Use 'vestige workspace new <name>' to create one.".dimmed());
            }
        }
        _ => anyhow::bail!("Unknown action '{}'. Use: set, get, list, new, clear", action),
    }
    Ok(())
}

fn auto_categorize_tags(content: &str, tags: &mut Vec<String>) {
    let existing_prefixes: Vec<String> = tags.iter()
        .filter_map(|t| t.split_once(':').map(|(p, _)| p.to_string()))
        .collect();
    let lower = content.to_lowercase();
    let has = |kw: &str| lower.contains(kw);
    let candidates: &[(&str, &str, &[&str])] = &[
        ("tool", "docker", &["docker", "container"]),
        ("tool", "kubernetes", &["kubernetes", "k8s"]),
        ("tool", "terraform", &["terraform"]),
        ("tool", "git", &["git commit", "git branch", "git push"]),
        ("tool", "aws", &["aws ", "s3 ", "lambda", "cloudformation"]),
        ("tool", "postgres", &["postgres", "postgresql"]),
        ("tool", "redis", &["redis"]),
        ("domain", "auth", &["auth", "login", "session", "oauth"]),
        ("domain", "devops", &["deploy", "ci/cd", "pipeline"]),
        ("domain", "testing", &["test ", "spec ", "assert"]),
        ("domain", "api", &["api ", "endpoint", "rest ", "graphql"]),
        ("domain", "data", &["database", "schema", "migration", "sql"]),
        ("domain", "performance", &["performance", "latency", "throughput"]),
        ("domain", "security", &["security", "vulnerability", "cve"]),
        ("lang", "typescript", &["typescript", ".ts ", ".tsx"]),
        ("lang", "rust", &["rust ", "cargo", ".rs "]),
        ("lang", "python", &["python", ".py ", "pip "]),
        ("lang", "go", &["golang", " go ", ".go "]),
        ("lang", "java", &["java ", "jvm", ".java"]),
    ];
    for (prefix, value, keywords) in candidates {
        if existing_prefixes.contains(&prefix.to_string()) { continue; }
        if keywords.iter().any(|kw| has(kw)) {
