...
impl McpServer {
    ...
    async fn handle_tools_list(&self) -> Result<serde_json::Value, JsonRpcError> {
        ...
        let tools = vec![
            ...
            ToolDescription {
                ...
                description: Some("List all tags across memories with counts. Tags use category prefixes (tool:, domain:, lang:, project:, intent:, entity:, trigger:). Returns tags sorted by frequency. Use to discover what the memory system knows about.".to_string()),
            ...
            // ================================================================
            ToolDescription {
                name: "export_claude_md".to_string(),
                description: Some("Export memories as a CLAUDE.md-formatted markdown document. Optionally filter by project (matches project: or codebase: tags). Groups by type: Decisions, Patterns, Preferences, Knowledge. Use for team knowledge sharing.".to_string()),
                input_schema: tools::export_claude::schema(),
            },
            // ================================================================
            // WORKSPACE (v2.1)
            // ================================================================
            ToolDescription {
                ...
                description: Some("Manage project workspaces. 'new' creates and activates a workspace — all future smart_ingest calls auto-tag with project:<name>. 'set' switches workspace. 'get' shows current. 'list' shows all. 'clear' returns to global mode. Each workspace becomes an independent knowledge graph viewable on the dashboard.".to_string()),
                input_schema: tools::workspace::schema(),
            },
        ];
    ...
    async fn handle_tools_call(
        ...
        let result = match request.name.as_str() {
            ...
            // TAG CATEGORIES (v2.1)
            // ================================================================
            "list_tags" => tools::tags::execute(&self.storage, request.arguments).await,
            "export_claude_md" => tools::export_claude::execute(&self.storage, request.arguments).await,
            "workspace" => tools::workspace::execute(&self.storage, request.arguments).await,
...
mod tests {
    ...
    async fn test_tools_list_returns_all_tools() {
        ...
        // v2.1: 26 tools (24 from v2.0.5 + list_tags + export)
        assert_eq!(tools.len(), 26, "Expected exactly 26 tools in v2.1+");
        ...
        // Tag categories + export (v2.1)
        assert!(tool_names.contains(&"list_tags"));
        assert!(tool_names.contains(&"export_claude_md"));
    }
