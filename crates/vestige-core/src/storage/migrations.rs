...
pub const MIGRATIONS: &[Migration] = &[
    ...
    Migration {
        version: 11,
        description: "v2.1 Pinned Core Blocks: memory tier system (core/active/archive) + decay tuning",
        up: MIGRATION_V11_UP,
    },
    Migration {
        version: 12,
        description: "v2.1 Config store: key-value config for workspace, preferences",
        up: MIGRATION_V12_UP,
    },
];
...
/// V11: Pinned Core Memory Blocks + Decay Tuning
///
/// - `tier`: core (always loaded at session start), active (normal), archive (deprioritized)
/// - `decay_class`: permanent (never decay), normal (FSRS-6), ephemeral (4x faster decay)
const MIGRATION_V11_UP: &str = r#"
ALTER TABLE knowledge_nodes ADD COLUMN tier TEXT DEFAULT 'active';
ALTER TABLE knowledge_nodes ADD COLUMN decay_class TEXT DEFAULT 'normal';

CREATE INDEX IF NOT EXISTS idx_nodes_tier ON knowledge_nodes(tier);
CREATE INDEX IF NOT EXISTS idx_nodes_decay_class ON knowledge_nodes(decay_class);

UPDATE schema_version SET version = 11, applied_at = datetime('now');
"#;

/// V12: General config store
const MIGRATION_V12_UP: &str = r#"
CREATE TABLE IF NOT EXISTS vestige_config (
    key TEXT PRIMARY KEY,
    value TEXT NOT NULL,
    updated_at TEXT NOT NULL
);
