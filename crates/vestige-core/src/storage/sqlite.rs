...
impl Storage {
    ...
    pub fn get_config(&self, key: &str) -> Result<Option<String>> {
        ...
        let result: Option<String> = reader
            .query_row(
                "SELECT value FROM vestige_config WHERE key = ?1",
                params![key],
                |row| row.get(0),
            )
            .optional()?;
    ...
    pub fn set_config(&self, key: &str, value: &str) -> Result<()> {
        ...
        writer.execute(
            "INSERT OR REPLACE INTO vestige_config (key, value, updated_at) VALUES (?1, ?2, datetime('now'))",
            params![key, value],
        )?;
        Ok(())
    }

    pub fn reader_lock(&self) -> Result<std::sync::MutexGuard<'_, Connection>> {
        self.reader
            .lock()
            .map_err(|_| StorageError::Init("Reader lock poisoned".into()))
    }
    ...
    pub fn smart_ingest(&self, input: IngestInput) -> Result<SmartIngestResult> {
        ...
        match decision {
            ...
            GateDecision::Supersede {
                ...
                // Create a typed "supersedes" edge
                let now = Utc::now();
                let _ = self.save_connection(&ConnectionRecord {
                    source_id: node.id.clone(),
                    target_id: old_memory_id.clone(),
    ...
    fn row_to_node(row: &rusqlite::Row) -> rusqlite::Result<KnowledgeNode> {
        ...
        Ok(KnowledgeNode {
            ...
            suppression_count,
            suppressed_at,
            // v2.1 Pinned Core Blocks + Decay Tuning
            tier: row.get("tier").unwrap_or_else(|_| "active".to_string()),
            decay_class: row.get("decay_class").unwrap_or_else(|_| "normal".to_string()),
        })
    }
    ...
    pub fn apply_decay(&self) -> Result<i32> {
        ...
        loop {
            ...
            let batch: Vec<(String, String, f64, f64, f64, f64, String)> = {
                let reader = self
                    .reader
                    .lock()
                    .map_err(|_| StorageError::Init("Reader lock poisoned".into()))?;
                reader
                    .prepare(
                        "SELECT id, last_accessed, storage_strength, retrieval_strength,
                                ...
                                COALESCE(decay_class, 'normal') as decay_class
                         FROM knowledge_nodes
                         WHERE COALESCE(decay_class, 'normal') != 'permanent'
                         ORDER BY id
                         LIMIT ?1 OFFSET ?2",
                    )?
            ...
            // Write batch using writer transaction
            {
                ...
                for (id, last_accessed, storage_strength, _, sentiment_mag, stability, decay_class) in &batch {
                    ...
                    if days_since > 0.0 {
                        // Ephemeral memories decay 4x faster (simulate more elapsed time)
                        let effective_days = if decay_class == "ephemeral" {
                            days_since * 4.0
                        } else {
                            days_since
                        };
                        ...
                        let new_retrieval =
                            retrievability_with_decay(effective_stability, effective_days, w20);
...
impl Storage {
    ...
    pub fn set_memory_tier(&self, node_id: &str, tier: &str) -> Result<bool> {
        match tier {
            "core" | "active" | "archive" => {}
            _ => return Err(StorageError::Init(format!("Invalid tier: {}", tier))),
        }
        let writer = self
            .writer
            .lock()
        ...
        let updated = writer.execute(
            "UPDATE knowledge_nodes SET tier = ?1 WHERE id = ?2",
            params![tier, node_id],
        )?;
        Ok(updated > 0)
    }

    pub fn set_decay_class(&self, node_id: &str, decay_class: &str) -> Result<bool> {
        match decay_class {
            "permanent" | "normal" | "ephemeral" => {}
            _ => return Err(StorageError::Init(format!("Invalid decay_class: {}", decay_class))),
        }
        let writer = self
            .writer
            .lock()
            .map_err(|_| StorageError::Init("Writer lock poisoned".into()))?;
        let updated = writer.execute(
            "UPDATE knowledge_nodes SET decay_class = ?1 WHERE id = ?2",
            params![decay_class, node_id],
        )?;
        Ok(updated > 0)
    }

    pub fn get_triggered_memories(&self, context_terms: &[&str]) -> Result<Vec<KnowledgeNode>> {
        ...
        for term in context_terms {
            let pattern = format!("%trigger:{}%", term.to_lowercase());
            let mut stmt = reader.prepare(
                "SELECT * FROM knowledge_nodes WHERE LOWER(tags) LIKE ?1 LIMIT 5",
            )?;
            let nodes = stmt.query_map(params![pattern], Self::row_to_node)?;
            for node in nodes {
                if let Ok(n) = node {
    ...
    pub fn get_core_memories(&self) -> Result<Vec<KnowledgeNode>> {
        let reader = self
            .reader
            .lock()
            .map_err(|_| StorageError::Init("Reader lock poisoned".into()))?;
        let mut stmt = reader.prepare(
            "SELECT * FROM knowledge_nodes WHERE tier = 'core' ORDER BY updated_at DESC",
        )?;
        let nodes = stmt.query_map([], Self::row_to_node)?;
        let mut result = Vec::new();
        for node in nodes {
            result.push(node?);
        }
    ...
    pub fn list_tags(&self) -> Result<Vec<(String, i64)>> {
        let reader = self
            ...
            .lock()
            .map_err(|_| StorageError::Init("Reader lock poisoned".into()))?;
        let mut stmt = reader.prepare("SELECT tags FROM knowledge_nodes WHERE tags != '[]'")?;
        let rows = stmt.query_map([], |row| {
    ...
    pub fn get_at_risk_memories(&self, limit: i32) -> Result<Vec<KnowledgeNode>> {
        let reader = self
            .reader
            .lock()
            .map_err(|_| StorageError::Init("Reader lock poisoned".into()))?;
        let mut stmt = reader.prepare(
            "SELECT * FROM knowledge_nodes
             WHERE retention_strength < 0.4 AND storage_strength > 0.6
