...
export const api = {
	...
	// Tags / Workspaces
	tags: () => fetcher<{ tags: { tag: string; count: number }[]; workspaces: string[]; total: number }>('/tags'),

	// Analytics
	analytics: () => fetcher<Record<string, any>>('/analytics'),

	// Graph
	graph: (params?: { query?: string; center_id?: string; depth?: number; max_nodes?: number; workspace?: string }) => {
	...
	explore: (fromId: string, action = 'associations', toId?: string, limit = 10, maxHops?: number) =>
		fetcher<Record<string, unknown>>('/explore', {
			method: 'POST',
			body: JSON.stringify({ from_id: fromId, action, to_id: toId, limit, max_hops: maxHops })
		}),
	...
	createIntention: (content: string, opts?: { trigger_type?: string; priority?: number; deadline?: string }) =>
		fetcher<{ success: boolean; id: string }>('/intentions', {
			method: 'POST',
			body: JSON.stringify({ content, ...opts })
		}),
	updateIntention: (id: string, status: string) =>
		fetcher<{ success: boolean }>(`/intentions/${id}`, {
			method: 'PUT',
